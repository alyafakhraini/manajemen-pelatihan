...
if (isset($_GET['search_query'])) {
    ...
    $result_agenda = mysqli_query($conn, "SELECT * FROM tbl_agenda WHERE nama_agenda LIKE '%$searchQuery%' ORDER BY tgl_agenda LIMIT $mulai, $limit");
    $total_records_query = mysqli_query($conn, "SELECT COUNT(*) AS total FROM tbl_agenda WHERE nama_agenda LIKE '%$searchQuery%'");
} else {
    // Query default
    $result_agenda = mysqli_query($conn, "SELECT * FROM tbl_agenda ORDER BY tgl_agenda LIMIT $mulai, $limit");
    $total_records_query = mysqli_query($conn, "SELECT COUNT(*) AS total FROM tbl_agenda");
}
                            ...
                            <div class="card">
                                <div class="card-body">
                                    <div class="breadcrumb-container">
                                        <h3 class="mt-4"><b>Data Agenda</b></h3>
                                        <ol class="breadcrumb">
                                    ...
                                    <a href="agenda-tambah.php" class="btn btn-outline-primary btn-fw">Tambah Data</a>
                                            ...
                                            <tbody>
                                                ...
                                                while ($row_agenda = mysqli_fetch_assoc($result_agenda)) { ?>
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <?php
                                                            if (isset($row_agenda["tgl_agenda"])) {
                                                                $tgl_agenda = $row_agenda["tgl_agenda"];
                                                                $timestamp = strtotime($tgl_agenda);
                                                        ...
                                                        <td><?php echo $row_agenda["deskripsi"] ?></td>
                                                        <td><?php echo $row_agenda["tujuan_tempat"] ?></td>
                                                        <td>
                                                            <?php
                                                            if ($row_agenda["status_agenda"] == "on going") { //status 1 (on going)
                                                                echo "<div class='btn btn-sm btn-secondary'>On Going</div>";
                                                            } else if ($row_agenda["status_agenda"] == "done") { //status 2 (done)
                                                                echo "<div class='btn btn-sm btn-success'>Done</div>";
                                                            } else if ($row_agenda["status_agenda"] == "postponed") { //status 3 (postponed)
                                                                echo "<div class='btn btn-sm btn-dark'>Postponed</div>";
                                                            }
                                                            ?>
                                                        </td>
                                                        <td>
                                                            <a href="agenda-edit.php?id=<?php echo $row_agenda["id_agenda"]; ?>" class="btn btn-sm btn-outline-warning btn-fw">Edit</a>
                                                            <a href="#" class="btn btn-sm btn-outline-danger btn-fw delete-btn" data-id="<?php echo $row_agenda["id_agenda"]; ?>">Hapus</a>
                                                        </td>
                                                    </tr>
                                                <?php } ?>
                                            </tbody>
            ...
            $('.delete-btn').click(function(e) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        window.location = `agenda-hapus.php?id=${id_agenda}`;
                    }
                });
            });
        });
